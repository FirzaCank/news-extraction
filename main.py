...
GCS_INPUT_PATH = os.environ.get("GCS_INPUT_PATH", "input/input.csv")
GCS_OUTPUT_PATH = os.environ.get("GCS_OUTPUT_PATH", "output")
...
def get_latest_input_file(bucket_name, input_folder='input'):
...
def read_input_from_gcs(bucket_name, input_folder='input'):
    """Read URLs with ID and date from the latest CSV file in GCS input folder"""
...
def batch_scrape(url_data_list, token):
    ...
    for i, url_data in enumerate(url_data_list, 1):
        ...
        print(f"\n[{i}/{len(url_data_list)}] ID:{record_id} | {url[:70]}...")
...
def save_results_csv(results, output_dir='output', input_filename=None):
...
if __name__ == "__main__":
    ...
    if LOCAL_MODE:
        ...
        print(f"\n☁️  CLOUD MODE: Reading latest file from gs://{GCS_BUCKET_NAME}/input/")
        urls, input_filename = read_input_from_gcs(GCS_BUCKET_NAME, 'input')
